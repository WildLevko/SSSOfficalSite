...
let isMobile = window.innerWidth <= 768; // Проверяем, мобильное устройство или нет

// Если мобильное устройство, уменьшаем скорость
const mobileSpeedFactor = isMobile ? 0.75 : 1;
...
// Управление на телефоне (пальцем)
if (isMobile) {
...
function getRandomSpeed() {
    const speed = (Math.random() * (maxSpeed - initialSpeed) + initialSpeed) * mobileSpeedFactor;
    ...
    balls.forEach((ball, index) => {
        ...
        if (
            ...
            if (Math.abs(ball.dx) >= maxSpeed && Math.abs(ball.dy) >= maxSpeed) {
                ...
                balls.forEach((b) => {
                    b.dx = initialSpeed * (b.dx > 0 ? 1 : -1) * mobileSpeedFactor;
                    b.dy = initialSpeed * (b.dy > 0 ? 1 : -1) * mobileSpeedFactor;
                });
            }
        }
