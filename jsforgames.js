...
function chooseGameMode() {
    let choice = prompt("Select game mode: 1 - Free, 2 - Story");
    if (choice === '1') {
        freemod()
    } else if (choice === '2') {
        alert("Oops! Unfortunately, the story mode is still in development(");
        chooseGameMode();
    } else {
...
function storymod(){
    ...
    function update() {
        ...
        if (score >= 100 && level === 1) {
            level = 2;
            alert("Поздравляем! Вы перешли на 2 уровень!");
        }
